...
/**
 * The {@code BaseSaveDomainCrudGraphqlRestResource} interface defines a GraphQL mutation for saving a single entity.
 * It extends the {@code ParentDomainCrudGraphqlRestResource} interface and provides a method for saving an entity with the specified DTO input.
 ...
 */
public interface BaseSaveDomainCrudGraphqlRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseSaveDomainCrudGraphqlRestResource.class);

    /**
     * Saves a single entity based on the specified DTO input for the specified CRUD resource.
     *
     * @param input The DTO input for saving the entity.
     * @param token The authorization token for authentication (optional).
     * @return A {@code Mono} emitting a {@code DtoType} containing the saved DTO as a string.
    ...
    default Mono<@NonNull DtoType> save(@Argument("input") DtoInput input, @ContextValue(value = HttpHeaders.AUTHORIZATION, required = false) String token) throws BadRequestException, DomainNotFoundException {
        logger.debug("Request to save: dto {}", input);
