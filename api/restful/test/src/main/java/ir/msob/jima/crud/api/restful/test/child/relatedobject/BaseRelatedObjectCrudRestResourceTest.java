package ir.msob.jima.crud.api.restful.test.child.relatedobject;
...
public interface BaseRelatedObjectCrudRestResourceTest<
        ...
        extends BaseRelatedObjectCrudResourceTest<ID, RID, USER, CHILD, CHILD_C, CNT, D, DTO, C, Q, R, S, DP, CHILD_S, CHILD_DP>,
    ...
    default void updateByRelatedIdRequest(ID parentId, RID relatedId, @NotNull @Valid CHILD child, Assertable<DTO> assertable) {
        this.getWebTestClient()
                .put()
                .uri(String.format("%s/%s/%s/related-id/%s", getBaseUri(), parentId, getElement(), relatedId))
                .headers(this::prepareHeader)
