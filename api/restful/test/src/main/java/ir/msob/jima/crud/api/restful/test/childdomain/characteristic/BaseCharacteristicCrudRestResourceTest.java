package ir.msob.jima.crud.api.restful.test.childdomain.characteristic;
...
import ir.msob.jima.crud.test.childdomain.characteristic.BaseCharacteristicCrudResourceTest;
...
public interface BaseCharacteristicCrudRestResourceTest<
        ...
        extends BaseCharacteristicCrudResourceTest<ID, USER, CD, CC, D, DTO, C, Q, R, S, DP, CS, CDP>,
    ...
    @Override
    default void updateByKeyRequest(ID parentId, String key, @NotNull @Valid CD child, Assertable<DTO> assertable) {
        this.getWebTestClient()
                .put()
                .uri(String.format("%s/%s/%s/key/%s", getBaseUri(), parentId, getElement(), key))
                .headers(this::prepareHeader)
