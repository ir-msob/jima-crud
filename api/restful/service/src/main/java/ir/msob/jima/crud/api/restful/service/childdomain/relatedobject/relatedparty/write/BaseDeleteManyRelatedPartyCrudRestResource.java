...
public interface BaseDeleteManyRelatedPartyCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteManyRelatedPartyCrudRestResource.class);

    @DeleteMapping("{parentId}/related-party/" + Operations.DELETE_MANY)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTO of deleted parties or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteMany(
        ...
        logger.debug("REST request to delete related parties by criteria, parentId {}, criteria {}", parentId, childCriteria);
