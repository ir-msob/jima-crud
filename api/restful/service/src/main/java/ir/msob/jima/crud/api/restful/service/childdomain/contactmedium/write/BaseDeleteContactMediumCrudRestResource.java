...
public interface BaseDeleteContactMediumCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteContactMediumCrudRestResource.class);

    @DeleteMapping("{parentId}/contact-medium")
    @Operation(
            summary = "Delete contact medium(s) by criteria",
            description = "Deletes one or more contact medium(s) of the parent domain matching the given criteria"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> delete(
        ...
        logger.debug("REST request to delete contact medium by criteria, parentId {}, childCriteria {}", parentId, childCriteria);
