...
public interface BaseUpdateByIdRelatedPartyCrudRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseUpdateByIdRelatedPartyCrudRestResource.class);


    @PutMapping("{parentId}/related-party/{id}")
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_PARTY, operation = Operations.UPDATE_BY_ID)
    default ResponseEntity<Mono<DTO>> updateById(@PathVariable("parentId") ID parentId, @PathVariable("id") ID id, @RequestBody @Valid RP dto, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update child party, parentId {}, id {}, dto {}", parentId, id, dto);
