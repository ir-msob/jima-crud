...
public interface BaseDeleteByRelatedIdRelatedPartyCrudRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseDeleteByRelatedIdRelatedPartyCrudRestResource.class);

    @DeleteMapping("{parentId}/related-party/related-id/{relatedId}")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTO of deleted party or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_PARTY, operation = Operations.DELETE_BY_RELATED_ID)
    default ResponseEntity<Mono<DTO>> deleteByRelatedId(
        ...
        log.debug("REST request to delete related party by relatedId, parentId {}, relatedId {}", parentId, relatedId);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.DELETE_BY_RELATED_ID)
                .body(getChildService().<String, CD, CC>deleteByRelatedId(parentId, relatedId, getChildDomainClass(), user));
    }
}
