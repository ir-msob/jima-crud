...
public interface BaseDeleteByIdRelatedProcessCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteByIdRelatedProcessCrudRestResource.class);

    @DeleteMapping("{parentId}/related-process/{id}")
    @Operation(
            summary = "Delete related process by ID",
            description = "Delete a related process child domain by its ID for a given parent ID"
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTO of deleted process or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_PROCESS, operation = Operations.DELETE_BY_ID)
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteById(
        ...
        logger.debug("REST request to delete childdomain process by id, parentId {}, id {}", parentId, id);
