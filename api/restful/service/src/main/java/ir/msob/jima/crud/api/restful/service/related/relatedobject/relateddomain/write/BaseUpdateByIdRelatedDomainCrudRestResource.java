...
public interface BaseUpdateByIdRelatedDomainCrudRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseUpdateByIdRelatedDomainCrudRestResource.class);


    @PutMapping("{parentId}/related-domain/{id}")
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_DOMAIN, operation = Operations.UPDATE_BY_ID)
    default ResponseEntity<Mono<DTO>> updateById(@PathVariable("parentId") ID parentId, @PathVariable("id") ID id, @RequestBody @Valid RD dto, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update related domain, parentId {}, id {}, dto {}", parentId, id, dto);
