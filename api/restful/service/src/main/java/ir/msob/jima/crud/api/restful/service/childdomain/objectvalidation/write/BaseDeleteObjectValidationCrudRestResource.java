...
public interface BaseDeleteObjectValidationCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteObjectValidationCrudRestResource.class);

    @DeleteMapping("{parentId}/object-validation")
    @Operation(
            summary = "Delete object validations by criteria",
            description = "Deletes object validations under the specified parent domain using criteria"
    )
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.OBJECT_VALIDATION, operation = Operations.DELETE)
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> delete(
        ...
        logger.debug("REST request to delete object-validations by criteria, parentId {}, childCriteria {}", parentId, childCriteria);
