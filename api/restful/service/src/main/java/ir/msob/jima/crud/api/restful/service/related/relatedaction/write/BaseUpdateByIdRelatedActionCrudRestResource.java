...
public interface BaseUpdateByIdRelatedActionCrudRestResource<
    ...
    @PutMapping("{parentId}/related-action/{id}")
    @ApiResponses(value = {
    ...
    default ResponseEntity<Mono<DTO>> updateById(@PathVariable("parentId") ID parentId, @PathVariable("id") ID id, @RequestBody @Valid OV dto, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update related action, parentId {}, id {}, dto {}", parentId, id, dto);
