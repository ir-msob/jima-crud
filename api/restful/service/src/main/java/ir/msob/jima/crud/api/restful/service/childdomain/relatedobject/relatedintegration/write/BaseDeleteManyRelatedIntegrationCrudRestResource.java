...
public interface BaseDeleteManyRelatedIntegrationCrudRestResource<
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_INTEGRATION, operation = Operations.DELETE_MANY)
    default ResponseEntity<Mono<DTO>> deleteMany(@PathVariable("parentId") ID parentId, CC childCriteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to delete childdomain integration by criteria, parentId {}, id {}", parentId, childCriteria);
