...
public interface BaseSaveContactMediumCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseSaveContactMediumCrudRestResource.class);

    @PostMapping("{parentId}/contact-medium")
    @Operation(
            summary = "Save a contact medium",
            description = "Creates a new contact medium under the specified parent domain"
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return ID of saved domain or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    @MethodStats
    @Scope(element = Elements.CONTACT_MEDIUM, operation = Operations.SAVE)
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> save(
        ...
        logger.debug("REST request to save contact medium, parentId {}, childDomain {}", parentId, childDomain);
