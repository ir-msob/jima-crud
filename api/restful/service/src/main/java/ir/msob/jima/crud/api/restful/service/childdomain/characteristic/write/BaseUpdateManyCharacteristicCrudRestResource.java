...
public interface BaseUpdateManyCharacteristicCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateManyCharacteristicCrudRestResource.class);

    @PutMapping("{parentId}/characteristic/" + Operations.UPDATE_MANY)
    @Operation(
            summary = "Update many characteristics",
            description = "Updates multiple characteristics for the given parent domain"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> updateMany(
        ...
        logger.debug("REST request to update many characteristics, parentId {}, childDomains {}", parentId, childDomains);
