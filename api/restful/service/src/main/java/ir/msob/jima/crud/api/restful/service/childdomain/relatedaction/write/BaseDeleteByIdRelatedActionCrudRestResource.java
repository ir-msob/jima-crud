...
public interface BaseDeleteByIdRelatedActionCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteByIdRelatedActionCrudRestResource.class);

    @DeleteMapping("{parentId}/related-action/" + Operations.DELETE_BY_ID + "/{id}")
    @Operation(
            summary = "Delete related action by id",
            description = "Deletes a related action under the specified parent domain by its identifier"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteById(
        ...
        logger.debug("REST request to delete related action by id, parentId {}, id {}", parentId, id);
