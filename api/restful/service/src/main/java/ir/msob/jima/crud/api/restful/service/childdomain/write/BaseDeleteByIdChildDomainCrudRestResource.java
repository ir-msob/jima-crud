...
import ir.msob.jima.crud.service.childdomain.write.BaseDeleteChildDomainCrudService;
 ...
 */
public interface BaseDeleteByIdChildDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteByIdChildDomainCrudRestResource.class);
    ...
    default ResponseEntity<@NonNull Mono<@NonNull ID>> deleteById(@PathVariable("parentDomainId") ID parentId, @PathVariable("id") ID id, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to delete domain by ID: {}", id);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull ID> res = this.getService().delete(parentId, id, user);
