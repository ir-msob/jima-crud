package ir.msob.jima.crud.api.restful.service.child.relatedobject.relatedparty.write;
...
public interface BaseUpdateManyRelatedPartyCrudRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseUpdateManyRelatedPartyCrudRestResource.class);


    @PutMapping("{parentId}/related-party/" + Operations.UPDATE_MANY)
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_PARTY, operation = Operations.UPDATE_MANY)
    default ResponseEntity<Mono<DTO>> updateMany(@PathVariable("parentId") ID parentId, @RequestBody Collection<@Valid RP> dtos, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update child party, parentId {}, dtos {}", parentId, dtos);
