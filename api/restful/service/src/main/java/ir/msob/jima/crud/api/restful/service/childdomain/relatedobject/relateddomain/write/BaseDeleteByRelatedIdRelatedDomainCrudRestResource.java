...
public interface BaseDeleteByRelatedIdRelatedDomainCrudRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseDeleteByRelatedIdRelatedDomainCrudRestResource.class);
    ...
    default ResponseEntity<Mono<DTO>> deleteByRelatedId(@PathVariable("parentId") ID parentId, @PathVariable("relatedId") ID relatedId, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to delete childdomain domain by relatedId, parentId {}, id {}", parentId, relatedId);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.DELETE_BY_RELATED_ID).body(getChildService().<ID, CD, CC>deleteByRelatedId(parentId, relatedId, getChildDomainClass(), user));
