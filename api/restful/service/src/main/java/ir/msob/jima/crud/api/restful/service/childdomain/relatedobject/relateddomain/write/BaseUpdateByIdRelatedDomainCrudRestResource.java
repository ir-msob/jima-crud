...
public interface BaseUpdateByIdRelatedDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateByIdRelatedDomainCrudRestResource.class);

    @PutMapping("{parentId}/related-domain/{id}")
    @Operation(
            summary = "Update related domain by ID",
            description = "Updates a related domain under the specified parent domain by its ID"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> updateById(
        ...
        logger.debug("REST request to update related domain, parentId {}, id {}, childDomain {}", parentId, id, childDomain);
