...
public interface BaseDeleteByNameContactMediumCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteByNameContactMediumCrudRestResource.class);

    @DeleteMapping("{parentId}/contact-medium/name/{name}")
    @Operation(
            summary = "Delete contact medium by name",
            description = "Deletes a single contact medium of the parent domain by its name"
    )
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.CONTACT_MEDIUM, operation = Operations.DELETE_BY_NAME)
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteByName(
        ...
        logger.debug("REST request to delete contact medium by name, parentId {}, name {}", parentId, name);
