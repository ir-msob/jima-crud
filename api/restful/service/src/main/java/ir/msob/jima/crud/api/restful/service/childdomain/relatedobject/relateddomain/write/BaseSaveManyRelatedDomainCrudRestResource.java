...
public interface BaseSaveManyRelatedDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseSaveManyRelatedDomainCrudRestResource.class);

    @PostMapping("{parentId}/related-domain/" + Operations.SAVE_MANY)
    @Operation(
            summary = "Save many related domains",
            description = "Saves multiple related domains under the specified parent domain"
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTOs of saved domains or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> saveMany(
        ...
        logger.debug("REST request to save many related domains, parentId {}, childDomains {}", parentId, childDomains);
