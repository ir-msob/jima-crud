...
public interface BaseSaveManyRelatedDomainCrudRestResource<
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_DOMAIN, operation = Operations.SAVE_MANY)
    default ResponseEntity<Mono<DTO>> saveMany(@PathVariable("parentId") ID parentId, @RequestBody Collection<@Valid CD> childDomains, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to save many childdomain domains, parentId {}, childDomains {}", parentId, childDomains);
