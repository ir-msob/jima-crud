...
public interface BaseUpdateRelatedIntegrationCrudRestResource<
    ...
    @PutMapping("{parentId}/related-integration")
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_INTEGRATION, operation = Operations.UPDATE)
    default ResponseEntity<Mono<DTO>> update(@PathVariable("parentId") ID parentId, @RequestBody @Valid RI dto, C criteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update child integration, parentId {}, dto {}, criteria {}", parentId, dto, criteria);
