...
public interface BaseUpdateRelatedActionCrudRestResource<
    ...
    @Operation(
            summary = "Update related action",
            description = "Updates a related action under the specified parent domain based on criteria"
    )
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_ACTION, operation = Operations.UPDATE)
    default ResponseEntity<Mono<DTO>> update(
        ...
        log.debug(
                "REST request to update related action, parentId {}, childDomain {}, childCriteria {}",
                parentId, childDomain, childCriteria
        );

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.UPDATE)
                .body(getChildService().<CD, CC>update(parentId, childDomain, childCriteria, getChildDomainClass(), user));
    }
