...
public interface BaseUpdateRelatedActionCrudRestResource<
    ...
    default ResponseEntity<Mono<DTO>> update(@PathVariable("parentId") ID parentId, @RequestBody @Valid CD childDomain, CC childCriteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update childdomain action, parentId {}, childDomain {}, childCriteria {}", parentId, childDomain, childCriteria);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.UPDATE).body(getChildService().<CD, CC>update(parentId, childDomain, childCriteria, getChildDomainClass(), user));
    }
