...
public interface BaseUpdateByRelatedIdRelatedDomainCrudRestResource<
    ...
    default ResponseEntity<Mono<DTO>> updateByRelatedId(@PathVariable("parentId") ID parentId, @PathVariable("relatedId") ID relatedId, @RequestBody @Valid CD childDomain, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update childdomain domain, parentId {}, relatedId {}, childDomain {}", parentId, relatedId, childDomain);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.UPDATE_BY_RELATED_ID).body(getChildService().<ID, CD, CC>updateByRelatedId(parentId, relatedId, childDomain, getChildDomainClass(), user));
    }
