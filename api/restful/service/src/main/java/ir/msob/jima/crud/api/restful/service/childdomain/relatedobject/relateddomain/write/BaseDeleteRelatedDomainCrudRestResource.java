...
public interface BaseDeleteRelatedDomainCrudRestResource<
    ...
    @DeleteMapping("{parentId}/related-domain")
    @Operation(
            summary = "Delete related domains",
            description = "Deletes related domains under the specified parent domain based on given criteria"
    )
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_DOMAIN, operation = Operations.DELETE)
    default ResponseEntity<Mono<DTO>> delete(
        ...
        log.debug("REST request to delete related domains by criteria, parentId {}, criteria {}", parentId, childCriteria);
