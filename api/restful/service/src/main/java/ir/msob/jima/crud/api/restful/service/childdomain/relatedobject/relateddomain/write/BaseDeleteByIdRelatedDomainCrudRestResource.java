...
public interface BaseDeleteByIdRelatedDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteByIdRelatedDomainCrudRestResource.class);

    @DeleteMapping("{parentId}/related-domain/{id}")
    @Operation(
            summary = "Delete related domain by ID",
            description = "Deletes a related domain under the specified parent domain by its ID"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteById(
        ...
        logger.debug("REST request to delete related domain by id, parentId {}, id {}", parentId, id);
