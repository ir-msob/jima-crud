...
public interface BaseUpdateByNameObjectValidationCrudRestResource<
    ...
    @PutMapping("{parentId}/object-validation/name/{name}")
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.OBJECT_VALIDATION, operation = Operations.UPDATE_BY_NAME)
    default ResponseEntity<Mono<DTO>> updateByName(@PathVariable("parentId") ID parentId, @PathVariable("name") String name, @RequestBody @Valid CD childDomain, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update object validation, parentId {}, name {}, childDomain {}", parentId, name, childDomain);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.UPDATE_BY_NAME).body(getChildService().<CD, CC>updateByName(parentId, name, childDomain, getChildDomainClass(), user));
    }
