...
public interface BaseUpdateByNameObjectValidationCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateByNameObjectValidationCrudRestResource.class);

    @PutMapping("{parentId}/object-validation/name/{name}")
    @Operation(
            summary = "Update object validation by name",
            description = "Updates a single object validation under the specified parent domain by name"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> updateByName(
        ...
        logger.debug("REST request to update object validation, parentId {}, name {}, childDomain {}", parentId, name, childDomain);
