...
public interface BaseUpdateManyRelatedPartyCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateManyRelatedPartyCrudRestResource.class);

    @PutMapping("{parentId}/related-party/" + Operations.UPDATE_MANY)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTO of updated parties", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> updateMany(
        ...
        logger.debug("REST request to update many childdomain parties, parentId {}, childDomains {}", parentId, childDomains);
