...
public interface BaseSaveManyContactMediumCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseSaveManyContactMediumCrudRestResource.class);

    @PostMapping("{parentId}/contact-medium/" + Operations.SAVE_MANY)
    @Operation(
            summary = "Save multiple contact mediums",
            description = "Creates multiple contact mediums under the specified parent domain"
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return IDs of saved domains or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> saveMany(
        ...
        logger.debug("REST request to save many contact mediums, parentId {}, childDomains {}", parentId, childDomains);
