...
public interface BaseUpdateRelatedDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateRelatedDomainCrudRestResource.class);

    @PutMapping("{parentId}/related-domain")
    @Operation(
            summary = "Update related domain",
            description = "Updates a related domain under the specified parent domain based on criteria"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> update(
        ...
        logger.debug("REST request to update related domain, parentId {}, childDomain {}, childCriteria {}", parentId, childDomain, childCriteria);
