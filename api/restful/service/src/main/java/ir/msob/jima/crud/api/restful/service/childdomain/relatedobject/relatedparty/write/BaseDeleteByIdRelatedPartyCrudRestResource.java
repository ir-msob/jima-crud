...
public interface BaseDeleteByIdRelatedPartyCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteByIdRelatedPartyCrudRestResource.class);

    @DeleteMapping("{parentId}/related-party/{id}")
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_PARTY, operation = Operations.DELETE_BY_ID)
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteById(
        ...
        logger.debug("REST request to delete related party by id, parentId {}, id {}", parentId, id);
