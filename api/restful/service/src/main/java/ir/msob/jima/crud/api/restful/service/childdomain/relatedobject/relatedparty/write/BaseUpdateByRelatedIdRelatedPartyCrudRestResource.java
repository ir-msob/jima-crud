...
public interface BaseUpdateByRelatedIdRelatedPartyCrudRestResource<
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_PARTY, operation = Operations.UPDATE_BY_RELATED_ID)
    default ResponseEntity<Mono<DTO>> updateByRelatedId(@PathVariable("parentId") ID parentId, @PathVariable("relatedId") String relatedId, @RequestBody @Valid CD childDomain, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update childdomain party, parentId {}, relatedId {}, childDomain {}", parentId, relatedId, childDomain);
