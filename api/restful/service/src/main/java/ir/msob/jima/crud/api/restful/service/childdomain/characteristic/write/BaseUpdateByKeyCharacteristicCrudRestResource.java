...
public interface BaseUpdateByKeyCharacteristicCrudRestResource<
    ...
    default ResponseEntity<Mono<DTO>> updateByKey(@PathVariable("parentId") ID parentId, @PathVariable("key") String key, @RequestBody @Valid CD childDomain, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update characteristic, parentId {}, key {}, childDomain {}", parentId, key, childDomain);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.UPDATE_BY_KEY).body(getChildService().<CD, CC>updateByKey(parentId, key, childDomain, getChildDomainClass(), user));
    }
}