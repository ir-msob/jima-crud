...
public interface BaseDeleteByNameRelatedActionCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteByNameRelatedActionCrudRestResource.class);

    @DeleteMapping("{parentId}/related-action/" + Operations.DELETE_BY_NAME + "/{name}")
    @Operation(
            summary = "Delete related action by name",
            description = "Deletes a related action under the specified parent domain by its name"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteByName(
        ...
        logger.debug("REST request to delete related action by name, parentId {}, name {}", parentId, name);
