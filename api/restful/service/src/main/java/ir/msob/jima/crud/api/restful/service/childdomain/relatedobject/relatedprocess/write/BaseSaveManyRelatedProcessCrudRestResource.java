...
public interface BaseSaveManyRelatedProcessCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseSaveManyRelatedProcessCrudRestResource.class);

    @Operation(summary = "Save many related processes", description = "Save multiple related process child domains for a given parent ID")
    @PostMapping("{parentId}/related-process/" + Operations.SAVE_MANY)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTO of saved processes or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> saveMany(
        ...
        logger.debug("REST request to save many childdomain processes, parentId {}, childDomains {}", parentId, childDomains);
