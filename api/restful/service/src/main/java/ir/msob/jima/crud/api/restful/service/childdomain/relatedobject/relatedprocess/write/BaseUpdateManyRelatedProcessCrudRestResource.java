...
public interface BaseUpdateManyRelatedProcessCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateManyRelatedProcessCrudRestResource.class);

    @Operation(summary = "Update many related processes", description = "Update multiple related process child domains for a given parent ID")
    @PutMapping("{parentId}/related-process/" + Operations.UPDATE_MANY)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTO of updated processes or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> updateMany(
        ...
        logger.debug("REST request to update childdomain processes, parentId {}, childDomains {}", parentId, childDomains);
