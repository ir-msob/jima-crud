...
public interface BaseDeleteManyEmbeddedDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteManyEmbeddedDomainCrudRestResource.class);

    @DeleteMapping(Operations.DELETE_MANY)
    @Operation(
            summary = "Delete multiple embedded domains by criteria",
            description = "Deletes multiple embedded domains of a parent domain based on provided criteria"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteMany(
        ...
        logger.debug("REST request to delete multiple embedded domains by criteria, parentId {}, criteria {}", parentId, childCriteria);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.DELETE_MANY)
                .body(getEmbeddedDomainService().deleteMany(parentId, childCriteria, getEmbeddedDomainClass(), user));
    }
}
