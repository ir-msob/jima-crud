...
public interface BaseSaveManyRelatedActionCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseSaveManyRelatedActionCrudRestResource.class);

    @PostMapping("{parentId}/related-action/" + Operations.SAVE_MANY)
    @Operation(
            summary = "Save many related actions",
            description = "Saves multiple related actions under the specified parent domain"
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTO of saved domains or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> saveMany(
        ...
        logger.debug(
                "REST request to save many related actions, parentId {}, childDomains {}",
