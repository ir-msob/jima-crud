...
public interface BaseSaveManyRelatedActionCrudRestResource<
    ...
    @PostMapping("{parentId}/related-action/" + Operations.SAVE_MANY)
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_ACTION, operation = Operations.SAVE_MANY)
    default ResponseEntity<Mono<DTO>> saveMany(@PathVariable("parentId") ID parentId, @RequestBody Collection<@Valid CD> childDomains, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to save many childdomain-actions, parentId {}, childDomains {}", parentId, childDomains);
