...
public interface BaseUpdateByRelatedIdRelatedProcessCrudRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseUpdateByRelatedIdRelatedProcessCrudRestResource.class);
    ...
    default ResponseEntity<Mono<DTO>> updateByRelatedId(@PathVariable("parentId") ID parentId, @PathVariable("relatedId") String relatedId, @RequestBody @Valid CD childDomain, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update childdomain process, parentId {}, relatedId {}, childDomain {}", parentId, relatedId, childDomain);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.UPDATE_BY_RELATED_ID).body(getChildService().<String, CD, CC>updateByRelatedId(parentId, relatedId, childDomain, getChildDomainClass(), user));
    }
