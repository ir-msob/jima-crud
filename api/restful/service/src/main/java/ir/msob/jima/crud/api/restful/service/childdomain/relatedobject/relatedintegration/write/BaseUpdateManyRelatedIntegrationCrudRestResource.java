...
public interface BaseUpdateManyRelatedIntegrationCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateManyRelatedIntegrationCrudRestResource.class);

    @PutMapping("{parentId}/related-integration/" + Operations.UPDATE_MANY)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Return DTO of updated integrations or null", content = @Content(schema = @Schema(implementation = BaseDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> updateMany(
        ...
        logger.debug("REST request to update many related integrations, parentId {}, childDomains {}", parentId, childDomains);
