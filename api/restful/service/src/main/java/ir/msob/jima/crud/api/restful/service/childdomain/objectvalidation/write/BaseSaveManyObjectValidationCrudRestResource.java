...
public interface BaseSaveManyObjectValidationCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseSaveManyObjectValidationCrudRestResource.class);

    @PostMapping("{parentId}/object-validation/" + Operations.SAVE_MANY)
    @Operation(
            summary = "Save multiple object validations",
            description = "Creates multiple object validations under the specified parent domain"
    )
    @ApiResponses(value = {
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> saveMany(
        ...
        logger.debug("REST request to save many object-validations, parentId {}, childDomains {}", parentId, childDomains);
