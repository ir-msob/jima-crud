...
public interface BaseSaveManyObjectValidationCrudRestResource<
    ...
    @PostMapping("{parentId}/object-validation/" + Operations.SAVE_MANY)
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.OBJECT_VALIDATION, operation = Operations.SAVE_MANY)
    default ResponseEntity<Mono<DTO>> saveMany(@PathVariable("parentId") ID parentId, @RequestBody Collection<@Valid CD> childDomains, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to save many object-validations, parentId {}, childDomains {}", parentId, childDomains);
