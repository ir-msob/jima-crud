...
public interface BaseUpdateRelatedDomainCrudRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseUpdateRelatedDomainCrudRestResource.class);


    @PutMapping("{parentId}/related-domain")
    @ApiResponses(value = {
    ...
    @MethodStats
    @Scope(element = Elements.RELATED_DOMAIN, operation = Operations.UPDATE)
    default ResponseEntity<Mono<DTO>> update(@PathVariable("parentId") ID parentId, @RequestBody @Valid RD dto, C criteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        log.debug("REST request to update child domain, parentId {}, dto {}, criteria {}", parentId, dto, criteria);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.UPDATE).body(getChildService().update(parentId, dto, criteria, user));
    }
