 ...
 */
public interface BaseUpdateManyDomainCrudService<ID extends Comparable<ID> & Serializable, USER extends BaseUser, D extends BaseDomain<ID>, DTO extends BaseDomainDto<ID>, C extends BaseDomainCriteria<ID>, R extends BaseRepository<ID, D, C>> extends ParentWriteDomainCrudService<ID, USER, D, DTO, C, R> {
    ...
    @Transactional
    @MethodStats
    default @NonNull Collection<DTO> updateMany(Collection<@Valid DTO> dtos, USER user) {
        var previousDtos = getManyByDto(dtos, user);
    ...
    @Transactional
    @MethodStats
    default @NonNull Collection<DTO> updateMany(Collection<DTO> previousDtos, Collection<@Valid DTO> dtos, USER user) {
