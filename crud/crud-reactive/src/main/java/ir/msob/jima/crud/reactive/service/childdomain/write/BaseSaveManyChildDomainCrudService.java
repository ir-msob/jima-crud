 ...
 */
public interface BaseSaveManyChildDomainCrudService<ID extends Comparable<ID> & Serializable, USER extends BaseUser, D extends BaseChildDomain<ID>, DTO extends BaseChildDto<ID>, C extends BaseChildCriteria<ID>, R extends BaseReactiveRepository<ID, D, C>> extends ParentWriteChildDomainCrudService<ID, USER, D, DTO, C, R> {
    Logger logger = LoggerFactory.getLogger(BaseSaveManyChildDomainCrudService.class);
    ...
    @Transactional
    @MethodStats
    default Mono<@NonNull Collection<DTO>> saveMany(ID parentId, Collection<@Valid DTO> dtos, USER user) throws BadRequestException, DomainNotFoundException {
