package ir.msob.jima.crud.reactive.test.resource.childdomain.read;
...
/**
 * The {@code BaseGetOneChildDomainCrudReactiveResourceTest} interface defines test cases for the getOne functionality of a CRUD resource.
 * It extends the {@code ParentChildDomainCrudReactiveResourceTest} interface and provides methods to test the getOne operation for CRUD resources.
 ...
 */
public interface BaseGetOneChildDomainCrudReactiveResourceTest<
    ...
    @Test
    @Rollback
    default void getOne() throws BadRequestException, DomainNotFoundException, ExecutionException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        DTO savedDto = getDataProvider().saveNew();
        ID parentId = savedDto.getParentId();
        Long countBefore = getDataProvider().countDb();
        getOneRequest(parentId, savedDto, dto -> getDataProvider().assertGet(getDataProvider().getNewDto(), dto));
    ...
    @Test
    @Rollback
    default void getOneMandatory() throws BadRequestException, DomainNotFoundException, ExecutionException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        DTO savedDto = getDataProvider().saveNewMandatory();
        ID parentId = savedDto.getParentId();
        Long countBefore = getDataProvider().countDb();
        getOneRequest(parentId, savedDto, dto -> getDataProvider().assertMandatoryGet(this.getDataProvider().getMandatoryNewDto(), dto));
        getDataProvider().assertCount(countBefore);

    }

    void getOneRequest(ID parentId, DTO savedDto, Assertable<DTO> assertable);
}
