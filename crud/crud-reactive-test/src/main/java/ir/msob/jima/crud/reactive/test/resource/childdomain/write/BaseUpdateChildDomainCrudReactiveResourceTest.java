...
/**
 * The {@code BaseUpdateChildDomainCrudReactiveResourceTest} interface defines test cases for the update functionality of a CRUD resource.
 * It extends the {@code ParentChildDomainCrudReactiveResourceTest} interface and provides methods to test the update operation for CRUD resources.
 ...
 */
public interface BaseUpdateChildDomainCrudReactiveResourceTest<
    ...
    default void update() throws BadRequestException, DomainNotFoundException, ExecutionException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        updateRequest(parentId, savedDtoForUpdate, updatedDto -> getDataProvider().assertUpdate(savedDto, updatedDto));
    ...
    @Test
    @Rollback
    default void updateMandatory() throws BadRequestException, DomainNotFoundException, ExecutionException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        DTO savedDto = getDataProvider().saveNewMandatory();
        ID parentId = savedDto.getParentId();
        DTO savedDtoForUpdate = savedDto.copy();
        this.getDataProvider().updateMandatoryDto(savedDtoForUpdate);
        Long countBefore = getDataProvider().countDb();
        updateRequest(parentId, savedDtoForUpdate, updatedDto -> getDataProvider().assertMandatoryUpdate(savedDto, updatedDto));
        getDataProvider().assertCount(countBefore);
    }
     ...
     * @throws DomainNotFoundException If the domain is not found.
     */
    void updateRequest(ID parentId, DTO dto, Assertable<DTO> assertable);
}
