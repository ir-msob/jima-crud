...
/**
 * The {@code BaseEditManyChildDomainCrudResourceTest} interface defines test cases for the editMany functionality of a CRUD resource.
 * It extends the {@code ParentChildDomainCrudResourceTest} interface and provides methods to test the editMany operation for CRUD resources.
 ...
 */
public interface BaseEditManyChildDomainCrudResourceTest<
    ...
    default void editMany() throws BadRequestException, DomainNotFoundException, ExecutionException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        editManyRequest(parentId, savedDtoForUpdate, getDataProvider().getJsonPatch(),
                updatedDtos -> {
    ...
    default void editManyMandatory() throws BadRequestException, DomainNotFoundException, ExecutionException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        Long countBefore = getDataProvider().countDb();
        editManyRequest(parentId, savedDtoForUpdate, getDataProvider().getMandatoryJsonPatch(),
                updatedDtos -> {
                    DTO updatedDto = getDataProvider().getObjectMapper().convertValue(updatedDtos.stream().findFirst().orElseThrow(DomainNotFoundException::new), getDtoClass());
     ...
     * @throws DomainNotFoundException If the domain is not found.
     */
    void editManyRequest(ID parentId, DTO savedDto, JsonPatch jsonPatch, Assertable<Collection<DTO>> assertable);
}
