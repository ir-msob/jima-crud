...
public interface BaseGetByIdDomainCrudKafkaListener<
    ...
    Logger logger = LoggerFactory.getLogger(BaseGetByIdDomainCrudKafkaListener.class);
    ...
    @PostConstruct
    default void startGetById() {
        KafkaListenerUtil.startListener(getKafkaConsumerFactory(),
                ChannelUtil.getChannel(getDtoClass(), Operations.GET_BY_ID),
                getGroupId(),
                this::getById);
    }
    ...
    @Scope(operation = Operations.GET_BY_ID)
    @Transactional
    default void getById(String dto) {
        logger.debug("Received message for get by id: dto {}", dto);
        ...
        getService().getOne(message.getData().getId(), message.getUser())
                .subscribe(getOneDto -> getChannelMessagePublisher().sendCallbackDto(message, getOneDto, OperationsStatus.GET_BY_ID, message.getUser()));
    }
}