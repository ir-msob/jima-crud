 ...
 */
public interface BaseDeleteDomainCrudKafkaListener<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteDomainCrudKafkaListener.class);
    ...
    @PostConstruct
    default void startDelete() {
        KafkaListenerUtil.startListener(getKafkaConsumerFactory(),
                ChannelUtil.getChannel(getDtoClass(), Operations.DELETE),
                getGroupId(),
                this::delete);
    }
    ...
    @Scope(operation = Operations.DELETE)
    @Transactional
    default void delete(String dto) {
        logger.debug("Received message for delete: dto {}", dto);
        ...
        getService().delete(message.getData().getCriteria(), message.getUser())
                .subscribe(deletedId -> getChannelMessagePublisher().sendCallbackId(message, deletedId, OperationsStatus.DELETE, message.getUser()));
    }
}