...
import ir.msob.jima.crud.reactive.service.domain.write.BaseDeleteDomainCrudReactiveService;
 ...
 * It extends the ParentDomainCrudRsocketResource interface and provides a default implementation for the delete method.
 *
 ...
 * @author Yaqub Abdi
 */
public interface BaseDeleteDomainCrudRsocketResource<
        ...
        S extends BaseDeleteDomainCrudReactiveService<ID, USER, D, DTO, C, R>
        > extends ParentDomainCrudRsocketResource<ID, USER, D, DTO, C, R, S> {
    Logger logger = LoggerFactory.getLogger(BaseDeleteDomainCrudRsocketResource.class);
    ...
    default Mono<@NonNull ID> delete(@Payload String dto, @AuthenticationPrincipal Jwt principal) throws BadRequestException, DomainNotFoundException, JsonProcessingException {
        logger.debug("RSocket request to delete domain, dto {}", dto);
