...
import ir.msob.jima.crud.core.service.domain.read.BaseGetPageDomainCrudService;
...
public interface BaseGetPageDomainCrudRestResource<
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns a page of matching domain DTOs", content = @Content(schema = @Schema(implementation = BaseDomainDto.class))),
    ...
    default ResponseEntity<@NonNull PageDto<DTO>> getPage(C criteria, PageableDto pageable, HttpServletRequest request, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to get page of domains with criteria: {}", criteria);

        USER user = getUser(request, principal);
        Page<DTO> res = this.getService().getPage(criteria, pageable.toPageable(), user);
        return ResponseEntity.status(OperationsStatus.GET_PAGE).body(PageDto.from(res));
    }
}
