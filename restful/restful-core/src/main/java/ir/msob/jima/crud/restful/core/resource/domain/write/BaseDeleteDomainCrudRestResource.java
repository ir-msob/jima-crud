 ...
 */
public interface BaseDeleteDomainCrudRestResource<
    ...
    default ResponseEntity<@NonNull ID> delete(C criteria, HttpServletRequest request, Principal principal) throws BadRequestException, DomainNotFoundException, JsonProcessingException {
        logger.debug("REST request to delete domain with criteria: {}", criteria);

        USER user = getUser(request, principal);
        ID res = this.getService().delete(criteria, user);
