...
import ir.msob.jima.crud.core.service.domain.write.BaseDeleteManyDomainCrudService;
...
public interface BaseDeleteManyDomainCrudRestResource<
    ...
    default ResponseEntity<@NonNull Collection<ID>> deleteMany(C criteria, HttpServletRequest request, Principal principal) throws BadRequestException, DomainNotFoundException, JsonProcessingException {
        logger.debug("REST request to delete many domains with criteria: {}", criteria);

        USER user = getUser(request, principal);
        Collection<ID> res = this.getService().deleteMany(criteria, user);
