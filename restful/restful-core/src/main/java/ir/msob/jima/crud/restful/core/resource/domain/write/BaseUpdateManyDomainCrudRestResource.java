...
import ir.msob.jima.crud.core.service.domain.write.BaseUpdateManyDomainCrudService;
...
public interface BaseUpdateManyDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateManyDomainCrudRestResource.class);
    ...
    default ResponseEntity<@NonNull Collection<DTO>> updateList(@RequestBody Collection<DTO> dtos, HttpServletRequest request, Principal principal)
            throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to update many domains, dtos: {}", dtos);

        USER user = getUser(request, principal);
        Collection<DTO> res = this.getService().updateMany(dtos, user);
