 ...
 */
public interface BaseGetOneDomainCrudRestResource<
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns the matching domain DTO", content = @Content(schema = @Schema(implementation = BaseDomainDto.class))),
    ...
    default ResponseEntity<@NonNull DTO> getOne(C criteria, HttpServletRequest request, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to get one domain with criteria: {}", criteria);

        USER user = getUser(request, principal);
        DTO res = this.getService().getOne(criteria, user);
