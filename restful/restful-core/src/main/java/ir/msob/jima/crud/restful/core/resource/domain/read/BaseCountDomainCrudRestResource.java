...
import ir.msob.jima.crud.core.service.domain.read.BaseCountDomainCrudService;
...
public interface BaseCountDomainCrudRestResource<
        ...
        S extends BaseCountDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseCountDomainCrudRestResource.class);
    ...
    default ResponseEntity<@NonNull Long> count(C criteria, HttpServletRequest request, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to count domains with criteria: {}", criteria);

        USER user = getUser(request, principal);
        Long res = this.getService().count(criteria, user);
