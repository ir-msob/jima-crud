...
import ir.msob.jima.crud.core.service.domain.read.BaseGetOneDomainCrudService;
...
public interface BaseGetByIdDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseGetByIdDomainCrudRestResource.class);

    @GetMapping("{id}")
    @Operation(summary = "Get domain by ID", description = "Returns the domain DTO for the given ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns the domain DTO", content = @Content(schema = @Schema(implementation = BaseDomainDto.class))),
    ...
    default ResponseEntity<@NonNull DTO> getById(@PathVariable("id") ID id, HttpServletRequest request, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to get domain by id: {}", id);

        USER user = getUser(request, principal);
        DTO res = this.getService().getOne(id, user);
