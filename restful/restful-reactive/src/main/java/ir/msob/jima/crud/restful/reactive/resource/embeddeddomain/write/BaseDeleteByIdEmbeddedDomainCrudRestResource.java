...
public interface BaseDeleteByIdEmbeddedDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseDeleteByIdEmbeddedDomainCrudRestResource.class);
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    ...
                    content = @Content(schema = @Schema(implementation = BaseDomainDto.class))
            ),
            @ApiResponse(
                    responseCode = "400",
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> deleteById(@PathVariable("parentId") ID parentId, @PathVariable("id") ID id, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to delete characteristic by id, parentId {}, id {}", parentId, id);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.DELETE_BY_ID).body(getEmbeddedDomainService().<ED, EC>deleteById(parentId, id, getEmbeddedDomainClass(), user));
    }
}