...
import ir.msob.jima.crud.reactive.service.childdomain.write.BaseUpdateChildDomainCrudService;
 ...
 */
public interface BaseUpdateByIdChildDomainCrudRestResource<
        ...
        S extends BaseUpdateChildDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentChildDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseUpdateByIdChildDomainCrudRestResource.class);

    @PutMapping("{id}")
    @Operation(summary = "Update domain by ID", description = "Updates an existing domain using the provided DTO and ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully updated domain", content = @Content(schema = @Schema(implementation = BaseChildDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> updateById(@PathVariable("parentDomainId") ID parentId, @PathVariable("id") ID id, @RequestBody DTO dto, ServerWebExchange serverWebExchange, Principal principal)
            throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to update domain, id {}, dto: {}", id, dto);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull DTO> res = this.getService().update(parentId, id, dto, user);
