...
import ir.msob.jima.crud.reactive.service.childdomain.write.BaseDeleteManyChildDomainCrudService;
 ...
 */
public interface BaseDeleteManyChildDomainCrudRestResource<
        ...
        S extends BaseDeleteManyChildDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentChildDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseDeleteManyChildDomainCrudRestResource.class);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "If domains were successfully deleted, returns the IDs", content = @Content(schema = @Schema(implementation = BaseChildDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull Collection<ID>>> deleteMany(@PathVariable("parentDomainId") ID parentId, C criteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException, JsonProcessingException {
        logger.debug("REST request to delete many domains with criteria: {}", criteria);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull Collection<ID>> res = this.getService().deleteMany(parentId, criteria, user);
