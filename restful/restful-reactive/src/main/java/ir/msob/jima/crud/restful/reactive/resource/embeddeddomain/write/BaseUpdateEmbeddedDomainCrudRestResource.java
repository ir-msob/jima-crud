...
public interface BaseUpdateEmbeddedDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateEmbeddedDomainCrudRestResource.class);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Characteristic updated successfully", content = @Content(schema = @Schema(implementation = BaseDomainDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> update(
        ...
        logger.debug("REST request to update characteristic, parentId {}, embeddedDomain {}, childCriteria {}", parentId, embeddedDomain, childCriteria);

        USER user = getUser(serverWebExchange, principal);
        return ResponseEntity.status(OperationsStatus.UPDATE)
                .body(getEmbeddedDomainService().update(parentId, embeddedDomain, childCriteria, getEmbeddedDomainClass(), user));
    }
}
