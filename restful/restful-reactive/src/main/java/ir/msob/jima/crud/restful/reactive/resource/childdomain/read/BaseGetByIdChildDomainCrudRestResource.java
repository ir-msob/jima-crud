 ...
 */
public interface BaseGetByIdChildDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseGetByIdChildDomainCrudRestResource.class);

    @GetMapping("{id}")
    @Operation(summary = "Get domain by ID", description = "Returns the domain DTO for the given ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns the domain DTO", content = @Content(schema = @Schema(implementation = BaseChildDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> getById(@PathVariable("parentDomainId") ID parentId, @PathVariable("id") ID id, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to get domain by id: {}", id);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull DTO> res = this.getService().getOne(parentId, id, user);
