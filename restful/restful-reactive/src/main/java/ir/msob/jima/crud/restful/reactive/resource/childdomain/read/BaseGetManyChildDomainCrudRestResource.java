...
import ir.msob.jima.crud.reactive.service.childdomain.read.BaseGetManyChildDomainCrudService;
 ...
 */
public interface BaseGetManyChildDomainCrudRestResource<
        ...
        S extends BaseGetManyChildDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentChildDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseGetManyChildDomainCrudRestResource.class);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns the collection of matching domain DTOs", content = @Content(schema = @Schema(implementation = BaseChildDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull Collection<DTO>>> getMany(@PathVariable("parentDomainId") ID parentId, C criteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to get many domains with criteria: {}", criteria);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull Collection<DTO>> res = this.getService().getMany(parentId, criteria, user);
