...
import ir.msob.jima.crud.reactive.service.childdomain.write.BaseEditManyChildDomainCrudService;
 ...
 */
public interface BaseEditManyChildDomainCrudRestResource<
        ...
        S extends BaseEditManyChildDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentChildDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseEditManyChildDomainCrudRestResource.class);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "If domains were successfully edited, returns the updated domains", content = @Content(schema = @Schema(implementation = BaseChildDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull Collection<DTO>>> editMany(@PathVariable("parentDomainId") ID parentId, @RequestBody String dto, C criteria, ServerWebExchange serverWebExchange, Principal principal)
        ...
        Mono<@NonNull Collection<DTO>> res = this.getService().editMany(parentId, criteria, JsonPatch.fromJson(getService().getObjectMapper().readTree(dto)), user);
