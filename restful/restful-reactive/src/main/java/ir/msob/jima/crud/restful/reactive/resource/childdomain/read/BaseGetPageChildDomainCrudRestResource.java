...
import ir.msob.jima.crud.reactive.service.childdomain.read.BaseGetPageChildDomainCrudService;
 ...
 */
public interface BaseGetPageChildDomainCrudRestResource<
        ...
        S extends BaseGetPageChildDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentChildDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseGetPageChildDomainCrudRestResource.class);

    @GetMapping
    @Operation(summary = "Get page of domains by criteria", description = "Returns a page of domain DTOs matching the given criteria")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns a page of matching domain DTOs", content = @Content(schema = @Schema(implementation = BaseChildDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull PageDto<DTO>>> getPage(@PathVariable("parentDomainId") ID parentId, C criteria, PageableDto pageable, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to get page of domains with criteria: {}", criteria);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull PageDto<DTO>> res = this.getService().getPage(parentId, criteria, pageable.toPageable(), user)
                .map(PageDto::from);
