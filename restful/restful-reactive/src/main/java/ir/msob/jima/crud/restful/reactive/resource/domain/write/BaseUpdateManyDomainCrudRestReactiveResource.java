 ...
 */
public interface BaseUpdateManyDomainCrudRestReactiveResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseUpdateManyDomainCrudRestReactiveResource.class);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully updated domains", content = @Content(schema = @Schema(implementation = BaseDomainDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull Collection<DTO>>> updateList(@RequestBody Collection<DTO> dtos, ServerWebExchange serverWebExchange, Principal principal)
            throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to update many domains, dtos: {}", dtos);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull Collection<DTO>> res = this.getService().updateMany(dtos, user);
