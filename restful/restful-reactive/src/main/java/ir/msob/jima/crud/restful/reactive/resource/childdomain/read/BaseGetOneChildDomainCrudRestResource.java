...
import ir.msob.jima.crud.reactive.service.childdomain.read.BaseGetOneChildDomainCrudService;
...
/**
 * RESTful API for retrieving a single domain based on criteria.
 *
 ...
 */
public interface BaseGetOneChildDomainCrudRestResource<
        ...
        S extends BaseGetOneChildDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentChildDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseGetOneChildDomainCrudRestResource.class);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns the matching domain DTO", content = @Content(schema = @Schema(implementation = BaseChildDto.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> getOne(@PathVariable("parentDomainId") ID parentId, C criteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to get one domain with criteria: {}", criteria);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull DTO> res = this.getService().getOne(parentId, criteria, user);
