package ir.msob.jima.crud.restful.reactive.resource.childdomain.read;
...
import ir.msob.jima.crud.reactive.service.childdomain.read.BaseCountChildDomainCrudService;
...
/**
 * RESTful API for counting domains based on criteria.
 *
 ...
 */
public interface BaseCountChildDomainCrudRestResource<
        ...
        S extends BaseCountChildDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentChildDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseCountChildDomainCrudRestResource.class);

    @GetMapping(Operations.COUNT)
    @Operation(summary = "Count domains by criteria", description = "Returns the count of domain records matching the given criteria")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns the count of matching domains", content = @Content(schema = @Schema(implementation = Long.class))),
    ...
    default ResponseEntity<@NonNull Mono<@NonNull Long>> count(@PathVariable("parentDomainId") ID parentId, C criteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to count domains with criteria: {}", criteria);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull Long> res = this.getService().count(parentId, criteria, user);
