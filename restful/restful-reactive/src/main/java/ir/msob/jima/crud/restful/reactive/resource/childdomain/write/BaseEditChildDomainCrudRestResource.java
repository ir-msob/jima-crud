...
import ir.msob.jima.crud.reactive.service.childdomain.write.BaseEditChildDomainCrudService;
 ...
 */
public interface BaseEditChildDomainCrudRestResource<
    ...
    Logger logger = LoggerFactory.getLogger(BaseEditChildDomainCrudRestResource.class);
    ...
    default ResponseEntity<@NonNull Mono<@NonNull DTO>> edit(@PathVariable("parentDomainId") ID parentId, @RequestBody String dto, C criteria, ServerWebExchange serverWebExchange, Principal principal)
            throws BadRequestException, DomainNotFoundException, IOException {
        logger.debug("REST request to edit domain, dto: {}, criteria: {}", dto, criteria);

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull DTO> res = this.getService().edit(parentId, criteria, JsonPatch.fromJson(getService().getObjectMapper().readTree(dto)), user);
