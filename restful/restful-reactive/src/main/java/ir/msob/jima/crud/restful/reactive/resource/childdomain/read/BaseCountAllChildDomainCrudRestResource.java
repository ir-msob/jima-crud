...
import ir.msob.jima.crud.reactive.service.childdomain.read.BaseCountAllChildDomainCrudService;
 ...
 */
public interface BaseCountAllChildDomainCrudRestResource<
        ...
        S extends BaseCountAllChildDomainCrudService<ID, USER, D, DTO, C, R>
        > extends ParentChildDomainCrudRestResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseCountAllChildDomainCrudRestResource.class);
    ...
    default ResponseEntity<@NonNull Mono<@NonNull Long>> countAll(@PathVariable("parentDomainId") ID parentId, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to count all domains");

        USER user = getUser(serverWebExchange, principal);
        Mono<@NonNull Long> res = this.getService().countAll(parentId, user);
