...
import ir.msob.jima.crud.reactive.service.domain.read.BaseGetStreamDomainCrudReactiveService;
 ...
 */
public interface BaseGetStreamDomainCrudRestReactiveResource<
        ...
        S extends BaseGetStreamDomainCrudReactiveService<ID, USER, D, DTO, C, R>
        > extends ParentDomainCrudRestReactiveResource<ID, USER, D, DTO, C, R, S> {

    Logger logger = LoggerFactory.getLogger(BaseGetStreamDomainCrudRestReactiveResource.class);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Returns a stream of matching domain DTOs", content = @Content(schema = @Schema(implementation = BaseDomainDto.class))),
    ...
    default ResponseEntity<@NonNull Flux<@NonNull DTO>> getStream(C criteria, ServerWebExchange serverWebExchange, Principal principal) throws BadRequestException, DomainNotFoundException {
        logger.debug("REST request to get stream of domains with criteria: {}", criteria);

        USER user = getUser(serverWebExchange, principal);
        Flux<@NonNull DTO> res = this.getService().getStream(criteria, user);
