package ir.msob.jima.crud.test.child.objectvalidation;
...
public interface BaseObjectValidationCrudResourceTest<
        ...
        CHILD extends ObjectValidationAbstract<ID>,
        CHILD_C extends ObjectValidationCriteriaAbstract<ID, CHILD>,
        CNT extends BaseObjectValidationContainer<ID, CHILD>,

        D extends BaseDomain<ID>,
        DTO extends BaseDto<ID> & BaseObjectValidationContainer<ID, CHILD>,
        ...
        CHILD_S extends BaseObjectValidationCrudService<ID, USER, CHILD, CHILD_C, CNT, DTO>,
    ...
    default void deleteByName() throws BadRequestException, DomainNotFoundException, ExecutionException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        getChildService().save(savedDto.getId(), child, BaseObjectValidationContainer::getObjectValidations, getSampleUser())
                .subscribe(dto ->
                        deleteByNameRequest(dto.getId()
                                , dto.getObjectValidations().first().getName()
                                , getDeleteAssertable())
                );
    }
    ...
    default void updateByName() throws BadRequestException, DomainNotFoundException, ExecutionException, InterruptedException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        getChildService().save(savedDto.getId(), child, BaseObjectValidationContainer::getObjectValidations, getSampleUser())
                .subscribe(dto -> {
                            CHILD toUpdate = dto.getObjectValidations().first();
    ...
    default Function<DTO, SortedSet<CHILD>> getChildGetter() {
        return DTO::getObjectValidations;
    }
}