package ir.msob.jima.crud.test.childdomain.characteristic;
...
public interface BaseCharacteristicCrudResourceTest<
        ...
        getChildService().save(savedDto.getId(), child, getChildDomainClass(), getSampleUser())
                .subscribe(dto ->
                        deleteByKeyRequest(dto.getId()
                                , ChildDomainUtil.getFunction(getChildDomainClass(), getDtoClass()).apply(dto).first().getKey()
                                , getDeleteAssertable())
                );
    }
    ...
    void updateByKeyRequest(@NotNull ID parentId, @NotBlank String key, @NotNull @Valid CD child, Assertable<DTO> assertable);
